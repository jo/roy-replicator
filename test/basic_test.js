  ...
  'one change': function(test) {
    ...
    source.insert({ foo: 'bar' }, function() {
      roy.replicate({ source: source, target: target }, function(err, response) {
        ...
        test.equal(response.replication_id_version, 3, '"replication_id_version" should be "3"');
        ...
        test.equal(typeof result.end_last_seq, 'number', 'result should have a "end_last_seq" of type number');
        test.equal(result.end_last_seq, 1, '"end_last_seq" should be "1"');
        test.equal(typeof result.recorded_seq, 'number', 'result should have a "recorded_seq" of type number');
        test.equal(result.recorded_seq, 1, '"recorded_seq" should be "1"');
